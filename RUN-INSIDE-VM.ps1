...
function Set-GPUSpoofing {
    Write-Host "`n  # ================== SPOOFING GPU INFORMATION ==================" -ForegroundColor Magenta
    ...
    $gpuProfiles = @{
        ...
        "2" = @{
            ...
            ProviderName = "NVIDIA"
            AdapterString = "NVIDIA GeForce RTX 3080"
            BiosString = "Version 94.02.42.00.15"
            ChipType = "NVIDIA GeForce RTX 3080"
            DacType = "Integrated RAMDAC"
            MemorySize = 0x280000000
            DriverVersion = "31.0.15.3623"
            DriverDate = "1-18-2024"
        }
        "3" = @{
            ...
            DriverDesc = "NVIDIA GeForce GTX 1660 Super"
            ProviderName = "NVIDIA"
            AdapterString = "NVIDIA GeForce GTX 1660 Super"
            BiosString = "Version 90.16.35.00.08"
            ChipType = "NVIDIA GeForce GTX 1660 Super"
            DacType = "Integrated RAMDAC"
            MemorySize = 0x180000000
            DriverVersion = "31.0.15.3623"
            DriverDate = "1-18-2024"
        }
        "4" = @{
            ...
            AdapterString = "AMD Radeon RX 7900 XTX"
            BiosString = "113-D7020100-101"
            ChipType = "AMD Radeon Graphics Processor"
            DacType = "Internal DAC(400MHz)"
            MemorySize = 0x600000000
            DriverVersion = "31.0.21003.12011"
            DriverDate = "12-7-2023"
        }
        "5" = @{
            Manufacturer = "AMD"
            ...
            ProviderName = "Advanced Micro Devices, Inc."
            AdapterString = "AMD Radeon RX 6800 XT"
            BiosString = "113-D41207-X62"
            ChipType = "AMD Radeon Graphics Processor"
            DacType = "Internal DAC(400MHz)"
            MemorySize = 0x400000000
            DriverVersion = "31.0.21003.12011"
            DriverDate = "12-7-2023"
        }
    }
    
    Write-Host "`n  # Available GPU profiles:" -ForegroundColor Cyan
    foreach ($key in $gpuProfiles.Keys | Sort-Object) {
        Write-Host "  # $key. $($gpuProfiles[$key].Model)" -ForegroundColor Gray
    }
    
    $gpuChoice = Read-Host "`n  # Select GPU profile (1-5)"
    if (-not $gpuProfiles.ContainsKey($gpuChoice)) { $gpuChoice = "3" }
    
    $gpu = $gpuProfiles[$gpuChoice]
    
    Write-Host "  # [INFO] Applying GPU profile: $($gpu.Model)" -ForegroundColor Yellow
    ...
    if (Test-Path $videoClassPath) {
        Get-ChildItem $videoClassPath -ErrorAction SilentlyContinue | ForEach-Object {
            $subkeyPath = $_.PSPath
            $subkeyName = $_.PSChildName
            
            if ($subkeyName -match '^\d+$') {
                Write-Host "  # [INFO] Modifying video adapter at index $subkeyName..." -ForegroundColor Yellow
                
                try {
                    Set-ItemProperty -Path $subkeyPath -Name "DriverDesc" -Value $gpu.DriverDesc -Force -ErrorAction SilentlyContinue
                    Set-ItemProperty -Path $subkeyPath -Name "ProviderName" -Value $gpu.ProviderName -Force -ErrorAction SilentlyContinue
                    Set-ItemProperty -Path $subkeyPath -Name "HardwareInformation.AdapterString" -Value $gpu.AdapterString -Force -ErrorAction SilentlyContinue
                    Set-ItemProperty -Path $subkeyPath -Name "HardwareInformation.BiosString" -Value $gpu.BiosString -Force -ErrorAction SilentlyContinue
                    Set-ItemProperty -Path $subkeyPath -Name "HardwareInformation.ChipType" -Value $gpu.ChipType -Force -ErrorAction SilentlyContinue
                    Set-ItemProperty -Path $subkeyPath -Name "HardwareInformation.DacType" -Value $gpu.DacType -Force -ErrorAction SilentlyContinue
                    Set-ItemProperty -Path $subkeyPath -Name "HardwareInformation.MemorySize" -Value $gpu.MemorySize -Type QWord -Force -ErrorAction SilentlyContinue
                    Set-ItemProperty -Path $subkeyPath -Name "DriverVersion" -Value $gpu.DriverVersion -Force -ErrorAction SilentlyContinue
                    Set-ItemProperty -Path $subkeyPath -Name "DriverDate" -Value $gpu.DriverDate -Force -ErrorAction SilentlyContinue
                    
                    Write-Host "  # [OK] Updated adapter at index $subkeyName" -ForegroundColor Green
                }
                catch {
                    Write-Host "  # [WARN] Could not update all properties for adapter $subkeyName" -ForegroundColor Yellow
                }
            }
        }
    }
    
    Write-Host "  # [INFO] Cleaning VirtualBox GPU references from PCI enumeration..." -ForegroundColor Yellow
    
    Get-ChildItem "HKLM:\SYSTEM\CurrentControlSet\Enum\PCI" -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
        $devicePath = $_.PSPath
        try {
            $deviceDesc = (Get-ItemProperty -Path $devicePath -Name "DeviceDesc" -ErrorAction SilentlyContinue).DeviceDesc
            if ($deviceDesc -and ($deviceDesc -like "*VirtualBox*" -or $deviceDesc -like "*VBox*" -or $deviceDesc -like "*SVGA*")) {
                Set-ItemProperty -Path $devicePath -Name "DeviceDesc" -Value $gpu.DriverDesc -Force -ErrorAction SilentlyContinue
                Write-Host "  # [OK] Replaced VBox GPU reference: $deviceDesc" -ForegroundColor Green
            }
        }
        catch { }
    }
    
    Write-Host "  # [INFO] Updating DirectX display information..." -ForegroundColor Yellow
    
    $dxKey = "HKLM:\SOFTWARE\Microsoft\DirectX"
    if (-not (Test-Path $dxKey)) {
        New-Item -Path $dxKey -Force | Out-Null
    }
    Set-ItemProperty -Path $dxKey -Name "InstalledVersion" -Value "4.09.00.0904" -Force -ErrorAction SilentlyContinue
    Set-ItemProperty -Path $dxKey -Name "Version" -Value "4.09.00.0904" -Force -ErrorAction SilentlyContinue
    
    Write-Host "  # [OK] GPU spoofing complete: $($gpu.Model)" -ForegroundColor Green
    Write-Host "  # [NOTE] A restart may be required for all changes to take effect." -ForegroundColor Yellow
    
    $Global:restartRequired = $true
}

function Set-WMIAntiDetection {
    Write-Host "`n  # ================== SPOOFING WMI QUERIES ==================" -ForegroundColor Magenta
    
    Write-Host "  # [INFO] Modifying WMI-accessible registry values..." -ForegroundColor Yellow
    
    try {
        $systemPath = "HKLM:\HARDWARE\DESCRIPTION\System\BIOS"
        Set-ItemProperty -Path $systemPath -Name "SystemManufacturer" -Value "Dell Inc." -Force
        Set-ItemProperty -Path $systemPath -Name "SystemProductName" -Value "OptiPlex 7090" -Force
        Set-ItemProperty -Path $systemPath -Name "SystemFamily" -Value "OptiPlex" -Force
        
        Write-Host "  # [OK] Win32_ComputerSystem source data modified" -ForegroundColor Green
    }
    catch {
        Write-Host "  # [WARN] Could not modify WMI ComputerSystem data: $_" -ForegroundColor Yellow
    }
    
    try {
        $bbPath = "HKLM:\HARDWARE\DESCRIPTION\System\BIOS"
        Set-ItemProperty -Path $bbPath -Name "BaseBoardManufacturer" -Value "Dell Inc." -Force
        Set-ItemProperty -Path $bbPath -Name "BaseBoardProduct" -Value "0YXT71" -Force
        Set-ItemProperty -Path $bbPath -Name "BaseBoardVersion" -Value "A03" -Force
        
        Write-Host "  # [OK] Win32_BaseBoard source data modified" -ForegroundColor Green
    }
    catch {
        Write-Host "  # [WARN] Could not modify BaseBoard data: $_" -ForegroundColor Yellow
    }
    
    try {
        $biosPath = "HKLM:\HARDWARE\DESCRIPTION\System\BIOS"
        $legitBiosVersions = @("1.12.0", "2.18.1", "A23", "F.42")
        $randomBiosVersion = $legitBiosVersions | Get-Random
        
        Set-ItemProperty -Path $biosPath -Name "BIOSVendor" -Value "Dell Inc." -Force
        Set-ItemProperty -Path $biosPath -Name "BIOSVersion" -Value $randomBiosVersion -Force
        Set-ItemProperty -Path $biosPath -Name "BIOSReleaseDate" -Value "06/15/2024" -Force
        
        Set-ItemProperty -Path $biosPath -Name "BIOSMajorRelease" -Value 2 -Type DWord -Force
        Set-ItemProperty -Path $biosPath -Name "BIOSMinorRelease" -Value 8 -Type DWord -Force
        
        Write-Host "  # [OK] Win32_BIOS source data modified" -ForegroundColor Green
    }
    catch {
        Write-Host "  # [WARN] Could not modify BIOS WMI data: $_" -ForegroundColor Yellow
    }
    
    try {
        $diskPath = "HKLM:\HARDWARE\DEVICEMAP\Scsi"
        $legitDiskModels = @(
        ...
        $randomDisk = $legitDiskModels | Get-Random
        
        for ($port = 0; $port -le 3; $port++) {
            for ($bus = 0; $bus -le 3; $bus++) {
                $scsiPath = "$diskPath\Scsi Port $port\Scsi Bus $bus\Target Id 0\Logical Unit Id 0"
                if (Test-Path $scsiPath) {
                    Set-ItemProperty -Path $scsiPath -Name "Identifier" -Value $randomDisk -Force
                    Set-ItemProperty -Path $scsiPath -Name "SerialNumber" -Value (Get-RandomString -Length 20) -Force
                    Write-Host "  # [OK] Modified disk at Port $port, Bus $bus" -ForegroundColor Green
                }
            }
        }
    }
    catch {
        Write-Host "  # [WARN] Could not modify all disk WMI data: $_" -ForegroundColor Yellow
    }
    
    Write-Host "  # [INFO] Clearing WMI repository cache..." -ForegroundColor Yellow
    try {
        ...
        Write-Host "  # [WARN] Could not restart WMI service: $_" -ForegroundColor Yellow
    }
    
    $Global:restartRequired = $true
}

function Remove-VMSpecificDLLs {
    Write-Host "`n  # ================== REMOVING VM-SPECIFIC DLLs ==================" -ForegroundColor Magenta
    
    $vmDLLs = @(
        "VBoxHook.dll", "VBoxMRXNP.dll", "VBoxTray.exe", "VBoxControl.exe",
        "VBoxService.exe", "VBoxGuest.sys", "VBoxMouse.sys", "VBoxSF.sys",
        "VBoxVideo.sys", "VBoxDisp.dll", "VBoxOGLarrayspu.dll", "VBoxOGLcrutil.dll",
        "VBoxOGLerrorspu.dll", "VBoxOGLfeedbackspu.dll", "VBoxOGLpackspu.dll",
        "VBoxOGLpassthroughspu.dll", "VBoxOGL.dll",
        
        "vmGuestLib.dll", "vmhgfs.dll", "vmhgfs.sys", "vmmouse.sys",
        "vmhgfs.dll", "vmci.sys", "vmusbmouse.sys", "vmx_svga.sys",
        "vmtools.dll", "vmtoolsd.exe", "vm3dservice.exe", "vmacthlp.exe",
        
        "qemu-ga.exe", "qga-vss.dll", "qemuwmi.dll",
        
        "vmbusvss.dll", "vmicvss.dll", "vmictimesync.dll"
    )
    ...
    foreach ($location in $locations) {
        if (Test-Path $location) {
            foreach ($dll in $vmDLLs) {
                $fullPath = Join-Path $location $dll
                if (Test-Path $fullPath) {
                    try {
                        $processName = [System.IO.Path]::GetFileNameWithoutExtension($dll)
                        Stop-Process -Name $processName -Force -ErrorAction SilentlyContinue
                        
                        takeown /F $fullPath /A | Out-Null
                        icacls $fullPath /grant "Administrators:F" | Out-Null
                        
                        $newName = $fullPath + ".bak_" + (Get-Date -Format "yyyyMMdd")
                        Move-Item -Path $fullPath -Destination $newName -Force
                        
                        Write-Host "  # [OK] Renamed: $dll" -ForegroundColor Green
                        $removedCount++
                    }
                    catch {
                        Write-Host "  # [WARN] Could not remove $dll`: $_" -ForegroundColor Yellow
                    }
                }
            }
        }
    ...
    Write-Host "`n  # [INFO] Checking loaded DLLs in running processes..." -ForegroundColor Yellow
    ...
    foreach ($proc in $processes) {
        try {
            $proc.Modules | ForEach-Object {
                ...
                if ($vmDLLs -contains $moduleName) {
                    $foundVMDLLs += "$($proc.Name) -> $moduleName"
                }
            }
        }
        catch {
    ...
    if ($foundVMDLLs.Count -gt 0) {
        Write-Host "  # [WARN] VM DLLs currently loaded in memory:" -ForegroundColor Yellow
        $foundVMDLLs | ForEach-Object { Write-Host "  #   - $_" -ForegroundColor Yellow }
        Write-Host "  # [INFO] Restart required to unload these from memory" -ForegroundColor Yellow
    }
    
    Write-Host "  # [OK] Processed $removedCount VM-specific files" -ForegroundColor Green
    $Global:restartRequired = $true
}

function Set-PowerCapabilitiesSpoofing {
    Write-Host "`n  # ================== SPOOFING POWER CAPABILITIES ==================" -ForegroundColor Magenta
    
    Write-Host "  # [INFO] Configuring advanced power states..." -ForegroundColor Yellow
    
    try {
        $powerKey = "HKLM:\SYSTEM\CurrentControlSet\Control\Power"
        
        Set-ItemProperty -Path $powerKey -Name "CsEnabled" -Value 1 -Type DWord -Force
        Set-ItemProperty -Path $powerKey -Name "HibernateEnabled" -Value 1 -Type DWord -Force
        Set-ItemProperty -Path $powerKey -Name "HibernateFileSizePercent" -Value 75 -Type DWord -Force
        ...
        Write-Host "  # [OK] Power schemes configured" -ForegroundColor Green
        
        Write-Host "  # [INFO] Creating virtual battery registry entries..." -ForegroundColor Yellow
        
        $batteryPath = "HKLM:\SYSTEM\CurrentControlSet\Services\battery"
		if (-not (Test-Path $batteryPath)) {
			Write-Host "  # [INFO] Creating battery service registry key..." -ForegroundColor Yellow
			New-Item -Path $batteryPath -Force -ErrorAction SilentlyContinue | Out-Null
		}
		if (Test-Path $batteryPath) {
			Set-ItemProperty -Path $batteryPath -Name "Start" -Value 1 -Type DWord -Force -ErrorAction SilentlyContinue
			Write-Host "  # [OK] Battery service configured" -ForegroundColor Green
		} else {
			Write-Host "  # [WARN] Could not configure battery service" -ForegroundColor Yellow
		}
        
        $acpiPath = "HKLM:\SYSTEM\CurrentControlSet\Services\ACPI\Parameters"
        if (-not (Test-Path $acpiPath)) {
            New-Item -Path $acpiPath -Force | Out-Null
        }
        Set-ItemProperty -Path $acpiPath -Name "BatteryPresent" -Value 1 -Type DWord -Force
        
        Write-Host "  # [OK] Power capabilities spoofed" -ForegroundColor Green
    }
    catch {
        Write-Host "  # [WARN] Could not configure all power settings: $_" -ForegroundColor Yellow
    }
    
    Write-Host "`n  # [INFO] Current power capabilities:" -ForegroundColor Cyan
    powercfg /availablesleepstates
    
    $Global:restartRequired = $true
}

function Hide-VMProcesses {
    Write-Host "`n  # ================== HIDING/REMOVING VM PROCESSES ==================" -ForegroundColor Magenta
    
    $vmProcesses = @(
        "VBoxTray", "VBoxService", "VBoxControl",
        
        "vmtoolsd", "vmwaretray", "vmwareuser", "vmacthlp", "vm3dservice",
    ...
    foreach ($procName in $vmProcesses) {
        ...
        if ($processes) {
            foreach ($proc in $processes) {
                try {
                    Write-Host "  # [INFO] Stopping process: $($proc.Name) (PID: $($proc.Id))" -ForegroundColor Yellow
    ...
    $vmServices = @(
        "VBoxService", "VBoxGuest", "VBoxMouse", "VBoxSF", "VBoxVideo",
        "VMTools", "vmware-tools", "qemu-ga",
        "vmickvpexchange", "vmicguestinterface", "vmicshutdown", "vmicheartbeat",
        "vmictimesync", "vmicrdv", "vmicvss"
    )
    
    foreach ($svcName in $vmServices) {
        $service = Get-Service -Name $svcName -ErrorAction SilentlyContinue
        if ($service) {
            try {
                Write-Host "  # [INFO] Disabling service: $svcName" -ForegroundColor Yellow
                ...
                Write-Host "  # [WARN] Could not disable service $svcName`: $_" -ForegroundColor Yellow
            }
        }
    }
    
    Write-Host "  # [OK] Stopped/disabled $stoppedCount VM processes/services" -ForegroundColor Green
    $Global:restartRequired = $true
}

function Set-AdvancedGPUCapabilities {
    Write-Host "`n  # ================== ADVANCED GPU CAPABILITIES SPOOFING ==================" -ForegroundColor Magenta
    
    Write-Host "  # [INFO] Configuring DirectX capabilities..." -ForegroundColor Yellow
    ...
    Set-ItemProperty -Path $dxKey -Name "Version" -Value "4.09.00.0904" -Force
    Set-ItemProperty -Path $dxKey -Name "InstalledVersion" -Value "4.09.00.0904" -Force
    
    Set-ItemProperty -Path $dxKey -Name "D3D_FEATURE_LEVEL" -Value 0xC100 -Type DWord -Force
    
    Write-Host "  # [INFO] Configuring OpenGL capabilities..." -ForegroundColor Yellow
    
    $glKey = "HKLM:\SOFTWARE\Khronos\OpenGL"
    if (-not (Test-Path $glKey)) {
        New-Item -Path $glKey -Force | Out-Null
    }
    Set-ItemProperty -Path $glKey -Name "Version" -Value "4.6.0" -Force
    
    Write-Host "  # [INFO] Configuring Vulkan capabilities..." -ForegroundColor Yellow
    
    $vkKey = "HKLM:\SOFTWARE\Khronos\Vulkan\ExplicitLayers"
    if (-not (Test-Path $vkKey)) {
        New-Item -Path $vkKey -Force | Out-Null
    }
    
    $gpuKey = "HKLM:\SYSTEM\CurrentControlSet\Control\Class\{4d36e968-e325-11ce-bfc1-08002be10318}\0000"
    
    if (Test-Path $gpuKey) {
        Set-ItemProperty -Path $gpuKey -Name "Acceleration.Level" -Value 0 -Type DWord -Force
        Set-ItemProperty -Path $gpuKey -Name "Capabilities" -Value 0xFFFFFFFF -Type DWord -Force
        Set-ItemProperty -Path $gpuKey -Name "CapabilityOverride" -Value 0 -Type DWord -Force
        
        Set-ItemProperty -Path $gpuKey -Name "DriverVersion" -Value "31.0.15.3623" -Force
        Set-ItemProperty -Path $gpuKey -Name "DriverDate" -Value "1-18-2024" -Force
        
        Set-ItemProperty -Path $gpuKey -Name "PixelShaderVersion" -Value "5.1" -Force
        Set-ItemProperty -Path $gpuKey -Name "VertexShaderVersion" -Value "5.1" -Force
        
        Set-ItemProperty -Path $gpuKey -Name "EnableGDI" -Value 1 -Type DWord -Force
        Set-ItemProperty -Path $gpuKey -Name "EnableHDR" -Value 1 -Type DWord -Force
    }
    
    Write-Host "  # [INFO] Cleaning GPU device tree..." -ForegroundColor Yellow
    
    $deviceClasses = "HKLM:\SYSTEM\CurrentControlSet\Control\DeviceClasses"
    $displayGUID = "{5b45201d-f2f2-4f3b-85bb-30ff1f953599}"
    
    $displayPath = Join-Path $deviceClasses $displayGUID
    if (Test-Path $displayPath) {
        Get-ChildItem $displayPath -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
            try {
                $deviceData = Get-ItemProperty -Path $_.PSPath -ErrorAction SilentlyContinue
                if ($deviceData) {
                    $deviceData.PSObject.Properties | ForEach-Object {
                        if ($_.Value -match "VBox|VMware|VirtualBox|SVGA") {
                            Write-Host "  # [FOUND] VM GPU reference in device class, removing..." -ForegroundColor Yellow
    ...
    Write-Host "  # [OK] Advanced GPU capabilities configured" -ForegroundColor Green
    $Global:restartRequired = $true
}

function Set-FirmwareTableSpoofing {
    Write-Host "`n  # ================== ADVANCED FIRMWARE TABLE SPOOFING ==================" -ForegroundColor Magenta
    
    Write-Host "  # [INFO] Analyzing current firmware tables..." -ForegroundColor Yellow
    
    $suspiciousACPI = @("VBOX", "BOCHS", "QEMU", "VMWARE")
    ...
    if (Test-Path $acpiPath) {
        Get-ChildItem $acpiPath -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
            $tableName = $_.PSChildName
            if ($suspiciousACPI | Where-Object { $tableName -match $_ }) {
                Write-Host "  # [WARN] Found suspicious ACPI table: $tableName" -ForegroundColor Red
                Write-Host "  # [INFO] Attempting to remove..." -ForegroundColor Yellow
                try {
                    Remove-Item -Path $_.PSPath -Recurse -Force
                    Write-Host "  # [OK] Removed ACPI table: $tableName" -ForegroundColor Green
                }
                catch {
                    Write-Host "  # [WARN] Could not remove $tableName - may require boot-time fix" -ForegroundColor Yellow
                }
            }
        }
    }
    ...
    $legitACPI = @(
        "FACP", "APIC", "FPDT", "FIDT", "MCFG", "HPET", "SSDT", "BGRT",
        "TPM2", "LPIT", "WSMT", "SLIC", "MSDM", "UEFI", "IVRS"
    )
    
    foreach ($table in $legitACPI) {
        $tablePath = Join-Path $acpiPath $table
        if (-not (Test-Path $tablePath)) {
            Write-Host "  # [INFO] Creating missing ACPI table: $table" -ForegroundColor Yellow
            try {
                New-Item -Path $tablePath -Force | Out-Null
                New-ItemProperty -Path $tablePath -Name "OEMTableID" -Value "DELL   " -PropertyType String -Force | Out-Null
                New-ItemProperty -Path $tablePath -Name "OEMRevision" -Value 0x01072009 -PropertyType DWord -Force | Out-Null
            }
            catch {}
        }
    }
    
    Write-Host "  # [INFO] Checking SMBIOS firmware data..." -ForegroundColor Yellow
    
    try {
        $smbios = Get-WmiObject -Class Win32_BIOS
        
        if ($smbios.Manufacturer -match "innotek|VirtualBox|VMware|QEMU|Bochs|Xen") {
            Write-Host "  # [WARN] SMBIOS shows VM vendor: $($smbios.Manufacturer)" -ForegroundColor Red
            Write-Host "  # [INFO] This must be fixed at the hypervisor level" -ForegroundColor Yellow
        }
        else {
            Write-Host "  # [OK] SMBIOS vendor appears legitimate: $($smbios.Manufacturer)" -ForegroundColor Green
        }
    }
    catch {
        Write-Host "  # [WARN] Could not query SMBIOS: $_" -ForegroundColor Yellow
    }
    
    Write-Host "  # [OK] Firmware table spoofing complete" -ForegroundColor Green
}

function Set-AudioDeviceSpoofing {
    Write-Host "`n  # ================== AUDIO DEVICE PRESENCE SPOOFING ==================" -ForegroundColor Magenta
    
    Write-Host "  # [INFO] Checking for audio devices..." -ForegroundColor Yellow
    
    $audioDevices = Get-WmiObject -Class Win32_SoundDevice
    
    if (-not $audioDevices -or $audioDevices.Count -eq 0) {
        Write-Host "  # [WARN] No audio devices detected - typical VM indicator!" -ForegroundColor Red
        Write-Host "  # [INFO] Creating virtual audio device entries..." -ForegroundColor Yellow
        ...
        $randomAudio = $audioNames | Get-Random
        
        $deviceGuid = [guid]::NewGuid().ToString()
        $devicePath = Join-Path $audioPath "Render\{$deviceGuid}"
        
        New-Item -Path $devicePath -Force | Out-Null
        New-ItemProperty -Path $devicePath -Name "DeviceState" -Value 1 -PropertyType DWord -Force | Out-Null
        New-ItemProperty -Path $devicePath -Name "FriendlyName" -Value $randomAudio -PropertyType String -Force | Out-Null
        
        Write-Host "  # [OK] Created virtual audio device: $randomAudio" -ForegroundColor Green
    }
    else {
        foreach ($device in $audioDevices) {
            if ($device.Name -match "VirtualBox|VMware|QEMU|Bochs") {
                Write-Host "  # [WARN] VM audio device detected: $($device.Name)" -ForegroundColor Yellow
                
                $soundPath = "HKLM:\SYSTEM\CurrentControlSet\Control\Class\{4d36e96c-e325-11ce-bfc1-08002be10318}"
                Get-ChildItem $soundPath -ErrorAction SilentlyContinue | ForEach-Object {
                    $desc = (Get-ItemProperty -Path $_.PSPath -Name "DriverDesc" -ErrorAction SilentlyContinue).DriverDesc
                    if ($desc -match "VirtualBox|VMware") {
                        Set-ItemProperty -Path $_.PSPath -Name "DriverDesc" -Value "Realtek High Definition Audio" -Force
                        Write-Host "  # [OK] Renamed audio driver: $desc -> Realtek HD Audio" -ForegroundColor Green
                    }
                }
            }
        }
    ...
    $audioServices = @("AudioSrv", "Audiosrv", "AudioEndpointBuilder")
    foreach ($svc in $audioServices) {
        ...
        if ($service) {
            if ($service.StartType -eq "Disabled") {
                ...
                Write-Host "  # [OK] Enabled audio service: $svc" -ForegroundColor Green
            }
        }
    }
...
function Set-PCIDeviceIDSpoofing {
    Write-Host "`n  # ================== PCI DEVICE ID SPOOFING ==================" -ForegroundColor Magenta
    ...
    $vmPCIVendors = @{
        ...
        "1AF4" = "QEMU (virtio)"
        "1234" = "QEMU (vga)"
        "5333" = "S3 Graphics (Bochs)"
    }
    
    Write-Host "  # [INFO] Scanning PCI devices for VM vendors..." -ForegroundColor Yellow
    
    $pciPath = "HKLM:\SYSTEM\CurrentControlSet\Enum\PCI"
    $foundVM = $false
    
    if (Test-Path $pciPath) {
        Get-ChildItem $pciPath -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
            $devicePath = $_.PSPath
            $deviceName = $_.PSChildName
            
            if ($deviceName -match "VEN_([0-9A-F]{4})") {
                $vendorID = $Matches[1]
                
                if ($vmPCIVendors.ContainsKey($vendorID)) {
                    $vmName = $vmPCIVendors[$vendorID]
                    Write-Host "  # [FOUND] VM PCI device: $vmName (Vendor: $vendorID)" -ForegroundColor Red
                    $foundVM = $true
                    
                    try {
                        $friendlyName = (Get-ItemProperty -Path $devicePath -Name "FriendlyName" -ErrorAction SilentlyContinue).FriendlyName
                        if ($friendlyName) {
                            Set-ItemProperty -Path $devicePath -Name "FriendlyName" -Value "Standard PCI Device" -Force
                            Write-Host "  # [OK] Renamed PCI device" -ForegroundColor Green
                        }
                    }
                    catch {
                        Write-Host "  # [WARN] Could not modify PCI device: $_" -ForegroundColor Yellow
                    }
                }
            }
        }
    ...
    if ($foundVM) {
        Write-Host "`n  # [WARN] VM-specific PCI devices found - these cannot be fully hidden from Windows" -ForegroundColor Red
        Write-Host "  # [INFO] Recommendation: Disable unused devices in VM settings" -ForegroundColor Yellow
    }
    else {
        Write-Host "  # [OK] No obvious VM PCI vendors detected" -ForegroundColor Green
    }
    
    $Global:restartRequired = $true
}

function Remove-VMMutexes {
    Write-Host "`n  # ================== REMOVING VM-SPECIFIC MUTEXES ==================" -ForegroundColor Magenta
    
    $vmMutexes = @(
        "Global\VBoxSharedFoldersMapper",
        "Global\VBoxVideo",
        "Global\VBoxTray",
        "Global\VMwareVMMutex",
        "Global\VMwareVMCJP",
        "Global\VMware*",
        "Global\QEMU*"
    )
    
    Write-Host "  # [INFO] Searching for VM mutexes in specific registry locations..." -ForegroundColor Yellow
    ...
    foreach ($basePath in $targetPaths) {
        $elapsed = ((Get-Date) - $searchStart).TotalSeconds
        if ($elapsed -gt $searchTimeout) {
            Write-Host "  # [WARN] Search timeout reached ($searchTimeout seconds), stopping mutex scan" -ForegroundColor Yellow
            break
        }
        ...
        Write-Host "  # [INFO] Scanning: $basePath" -ForegroundColor Gray
        
        try {
            Get-ChildItem -Path $basePath -Recurse -Depth 2 -ErrorAction SilentlyContinue | ForEach-Object {
                if (((Get-Date) - $searchStart).TotalSeconds -gt $searchTimeout) {
                    return
                }
                    ...
                    if ($props) {
                        $props.PSObject.Properties | ForEach-Object {
                            if ($_.Value -is [string]) {
                                foreach ($mutexPattern in $vmMutexes) {
                                    if ($_.Value -like $mutexPattern) {
                                        Write-Host "  # [FOUND] VM mutex reference: $($_.Name) = $($_.Value)" -ForegroundColor Yellow
                                        try {
                                            Remove-ItemProperty -Path $props.PSPath -Name $_.Name -Force -ErrorAction SilentlyContinue
                                            $foundCount++
                                        }
                                        catch {
                                            Write-Host "  # [WARN] Could not remove: $($_.Name)" -ForegroundColor Yellow
                                        }
                                        break
                                    }
                                }
            ...
            Write-Host "  # [WARN] Could not fully scan $basePath" -ForegroundColor Yellow
        }
    }
    
    if ($foundCount -gt 0) {
        Write-Host "  # [OK] Found and removed $foundCount VM mutex references" -ForegroundColor Green
    } else {
        Write-Host "  # [INFO] No VM mutex references found" -ForegroundColor Cyan
    }
    
    Write-Host "  # [OK] Mutex cleanup complete" -ForegroundColor Green
}
...
function Test-HardeningStatus {
    ...
    $wmiChoice = $Global:UserChoices.ContainsKey("WMI") -and $Global:UserChoices["WMI"]
    $wmiBios = (Get-WmiObject -Class Win32_BIOS).Manufacturer
    $test = @{
        Name = "WMI BIOS Vendor"
        Expected = if ($wmiChoice) { "Non-VM vendor" } else { "Not Modified" }
        Actual = $wmiBios
        Status = if (($wmiChoice -and $wmiBios -notmatch "innotek|VirtualBox|VMware|QEMU") -or (!$wmiChoice)) { "PASS" } else { "FAIL" }
    }
    $results += $test

    $audioChoice = $Global:UserChoices.ContainsKey("Audio") -and $Global:UserChoices["Audio"]
    $audioDevices = @(Get-WmiObject -Class Win32_SoundDevice).Count
    $test = @{
        Name = "Audio Devices Present"
        Expected = if ($audioChoice) { "1 or more" } else { "Not Modified" }
        Actual = "$audioDevices device(s)"
        Status = if (($audioChoice -and $audioDevices -gt 0) -or (!$audioChoice)) { "PASS" } else { "FAIL" }
    }
    $results += $test

    $pciChoice = $Global:UserChoices.ContainsKey("PCI") -and $Global:UserChoices["PCI"]
    $vmPCIFound = $false
    if (Test-Path "HKLM:\SYSTEM\CurrentControlSet\Enum\PCI") {
        Get-ChildItem "HKLM:\SYSTEM\CurrentControlSet\Enum\PCI" -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
            if ($_.PSChildName -match "VEN_(80EE|15AD|1AF4|1234)") {
                $vmPCIFound = $true
            }
        }
    }
    $test = @{
        Name = "VM PCI Devices"
        Expected = if ($pciChoice) { "Hidden/None" } else { "Not Modified" }
        Actual = if ($vmPCIFound) { "VM PCI found" } else { "No VM PCI" }
        Status = if (($pciChoice -and !$vmPCIFound) -or (!$pciChoice)) { "PASS" } else { "FAIL" }
    }
    $results += $test
    
    $powerChoice = $Global:UserChoices.ContainsKey("PowerCaps") -and $Global:UserChoices["PowerCaps"]
    ...
    $test = @{
        ...
        Expected = if ($powerChoice) { "Hibernate enabled" } else { "Not Modified" }
        Actual = if ($hibernateEnabled -eq 1) { "Hibernate enabled" } else { "Hibernate disabled" }
        Status = if (($powerChoice -and $hibernateEnabled -eq 1) -or (!$powerChoice)) { "PASS" } else { "FAIL" }
    }
    $results += $test
...
try {
    ...
    Write-Host "  +================================================================+" -ForegroundColor Red
    Write-Host "  | * This script makes IRREVERSIBLE changes to system identifiers |" -ForegroundColor Yellow
    Write-Host "  | * Windows activation may be affected by hardware changes       |" -ForegroundColor Yellow
    Write-Host "  | * Some antivirus software may flag these modifications         |" -ForegroundColor Yellow
    Write-Host "  | * Create a system restore point before proceeding              |" -ForegroundColor Yellow
    Write-Host "  +================================================================+" -ForegroundColor Red
    Write-Host "`n"
    ...
    Write-Host "  # --- SECTION 1: ANTI-VM DETECTION & SPOOFING ---" -ForegroundColor Green
	...
	Write-Host "`n`n  # ===========================================================" -ForegroundColor Green
	Write-Host "  # --- SECTION 2: ADDITIONAL ANTI-DETECTION TECHNIQUES ---" -ForegroundColor Green
	Write-Host "  # ===========================================================" -ForegroundColor Green

    if (Get-YesNoChoice "Apply WMI anti-detection spoofing?" "Y" "Modifies WMI-queryable hardware info." "WMI") {
        Set-WMIAntiDetection
        $Global:restartRequired = $true
    }

    if (Get-YesNoChoice "Remove VM-specific DLLs and drivers?" "Y" "Comprehensive removal of VM guest additions." "VMDLL") {
        Remove-VMSpecificDLLs
        $Global:restartRequired = $true
    }

    if (Get-YesNoChoice "Configure advanced power capabilities?" "Y" "Makes power management look realistic." "PowerCaps") {
        Set-PowerCapabilitiesSpoofing
        $Global:restartRequired = $true
    }

    if (Get-YesNoChoice "Hide/remove VM processes and services?" "Y" "Stops and disables all VM-related services." "VMProc") {
        Hide-VMProcesses
        $Global:restartRequired = $true
    }

    if (Get-YesNoChoice "Apply advanced GPU capability spoofing?" "Y" "DirectX/OpenGL/Vulkan configuration." "GPUCaps") {
        Set-AdvancedGPUCapabilities
        $Global:restartRequired = $true
    }

    if (Get-YesNoChoice "Spoof firmware and ACPI tables?" "Y" "Advanced firmware-level spoofing." "Firmware") {
        Set-FirmwareTableSpoofing
        $Global:restartRequired = $true
    }

    if (Get-YesNoChoice "Configure audio device presence?" "Y" "Creates/fixes audio device entries." "Audio") {
        Set-AudioDeviceSpoofing
        $Global:restartRequired = $true
    }

    if (Get-YesNoChoice "Scan and spoof PCI device IDs?" "Y" "Hides VM-specific PCI vendors." "PCI") {
        Set-PCIDeviceIDSpoofing
        $Global:restartRequired = $true
    }

    if (Get-YesNoChoice "Remove VM mutex objects?" "Y" "Cleans VM-specific synchronization objects." "Mutex") {
        Remove-VMMutexes
    }
	
    if (Get-YesNoChoice "Apply additional anti-detection fixes (Power, GPU, Display)?" "Y" "Fixes power capabilities, GPU detection, and display artifacts." "AdditionalAntiDetection") {
        Set-AdditionalAntiDetection
        $Global:restartRequired = $true
    }
    ...
    Write-Host "`n`n  # ═══════════════════════════════════════════════════════════" -ForegroundColor Green
    Write-Host "  # --- SECTION 3: STANDARD SYSTEM HARDENING ---" -ForegroundColor Green
    Write-Host "  # ═══════════════════════════════════════════════════════════" -ForegroundColor Green
    ...
    if (Get-YesNoChoice "Spoof GPU/graphics card information?" "Y" "Changes GPU name, VRAM, and driver info in registry" "GPUSpoof") {
        Set-GPUSpoofing
    }
